...
import lightning_lite.utilities.cloud_io as cloud_io
print(cloud_io._load.__code__.co_filename)
...
class UTMOSScore:
    ...
    def __init__(self, device, ckpt_path="epoch=3-step=7459.ckpt"):
        ...
        if not os.path.exists(filepath):
            ...
            print(f"Please download the UTMOS checkpoint from {UTMOS_CKPT_URL} and place it in {filepath}.")
...
def load_ssl_model(ckpt_path="wav2vec_small.pt"):
    filepath = os.path.join(os.path.dirname(__file__), ckpt_path)
    if not os.path.exists(filepath):
        print(f"Please download the Wav2Vec model from {WAV2VEC_URL} and place it in {filepath}.")
    SSL_OUT_DIM = 768
