...
class ResBlock1(nn.Module):
    ...
    def stable_snake(x: torch.Tensor, alpha: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        for c1, c2, gamma in zip(self.convs1, self.convs2, self.gamma):
            xt = stable_snake(x, getattr(self, 'snake_alpha', torch.tensor(1.0)))
            xt = c1(xt)
            xt = stable_snake(xt, getattr(self, 'snake_alpha', torch.tensor(1.0)))
            xt = c2(xt)
