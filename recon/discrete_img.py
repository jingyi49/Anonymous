...
from CosmosTokenizer.cosmos_tokenizer.modules.quantizers import InvQuantizerJit
...
class DiscreteImageTokenizer(nn.Module):
    ...
    def encode(self, x):
        h = self.encoder(x) #64, 256, 6, 6
        h = self.quant_conv(h) #64, 32, 6, 6
        return self.quantizer(h)

    def decode(self, quant):
        #shape check 3:torch.Size([64, 1, 1, 1])
    ...
    def forward(self, input):
        z_q, quant_loss, quant_info = self.encode(input)
        reconstructions = self.decode(z_q)
        if self.training:
            return dict(
